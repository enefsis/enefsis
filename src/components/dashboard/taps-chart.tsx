...
export function TapsChart({ data }: { data: ChartDay[] }) {
  const hasData = data.some(d => d.taps > 0)

  if (!hasData) {
    return (
      <div className="h-48 flex items-center justify-center">
        <p className="font-sans text-sm text-white/25">No tap data yet for this period</p>
      </div>
    )
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} barSize={8} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
        ...
        <Bar dataKey="taps" fill="#2B5CE6" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
